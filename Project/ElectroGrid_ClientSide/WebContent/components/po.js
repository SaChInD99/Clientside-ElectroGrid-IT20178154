...
{  
	...
	// Form validation-------------------  
	var status = validatePowerstationForm();  
	if (status != true)  
	{   
	...
	// If valid------------------------  
	var t = ($("#hidPowerstationIDSave").val() == "") ? "POST" : "PUT";
	
	$.ajax(
	{
		url : "PowerstationApi",
		type : t,
		data : $("#formPowerstation").serialize(),
		dataType : "text",
		complete : function(response, status)
		{
			onPowerstationSaveComplete(response.responseText, status);
		}
	});
}); 

function onPowerstationSaveComplete(response, status){
	if(status == "success")
	{
